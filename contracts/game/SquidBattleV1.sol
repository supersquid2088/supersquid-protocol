...
import "../tokenization/SuperSquidERC721.sol";
...
/**
 ...
 .#####:     ###.##  ##.###:    .#####:   ##                 .#####:    :###.##    ###.##  ########    .####
                     ...
                     ##                                                      ##
 */
contract SquidBattleV1 is OperatorAccessControl, ReentrancyGuard {
    ...
    event ClaimEvent(address indexed from, address indexed to, uint256 amount, uint256 type_);
    ...
    uint256 internal racingPriceAmount = 100e18;
    ...
    function setConfig(
        ...
        require(address(_rewardAddress) != address(0), "SetConfig: The reward address cannot be a 0 address");
        require(address(_erc721Address) != address(0), "SetConfig: The 721 address cannot be a 0 address");
        require(address(_erc20Address) != address(0), "SetConfig: The 20 address cannot be a 0 address");
     ...
     */
    function joinBattle(uint256[] memory _tokenIds) public nonReentrant returns (uint256 round_)
    {
        require(
            battleStartTime < block.timestamp,
        ...
        SuperSquidERC721 erc721 = SuperSquidERC721(erc721Address);
        ...
        uint256 nftPrice = erc721.getPrice(erc20Address);
     ...
     **/
    function liquidateBattle() public isOperatorOrOwner {
     ...
     * @param _tokenIds NFT tokenId
     **/
    function joinRacing(uint256 _amount, uint256[] memory _tokenIds) public nonReentrant returns (uint256 round_)
    {
        ...
        require(_amount >= racingPriceAmount * _tokenIds.length, "Racing: racingGamePrice error");

        IERC20 erc20 = IERC20(erc20Address);
        require(erc20.balanceOf(address(_msgSender())) >= _amount, "Racing: The ticket amount is not enough");
        ...
        erc20.transferFrom(_msgSender(), rewardAddress, _amount);
        GameBattleLib.BattleData storage battleDataInfo = bts2BattleHistory[battleStartTime];
        ...
        // record each user join game
        battleDataInfo.racingTotalTicketAmount = battleDataInfo.racingTotalTicketAmount + _amount;
     ...
     * @dev Liquidation Battle Job
     **/
    function liquidateRacing() public isOperatorOrOwner {
    ...
    function claim() public {
        ...
        require(erc20.balanceOf(rewardAddress) >= balance, "Game: game reward balance is not enough");
        ...
        erc20.transferFrom(rewardAddress, _msgSender(), userClaimAmount);
        erc20.transferFrom(rewardAddress, feeAddress, balance - userClaimAmount);

        emit ClaimEvent(rewardAddress, _msgSender(), userClaimAmount, 0);
        emit ClaimEvent(rewardAddress, feeAddress, balance - userClaimAmount, 1);
    }
